...
class DataManager:
    """
    CORREÇÃO CRÍTICA: As sequências agora são geradas DENTRO de cada split,
    eliminando o vazamento temporal entre treino, validação e teste.
    """
    ...
    def prepare_sequences(self, n_steps):
        """
        CORREÇÃO: Gera sequências independentemente para cada partição.
        """
...
# ======================== CONSTRUÇÃO DO MODELO (SIMPLIFICADA) ========================
    ...
    def _evaluate(self, model, history, exp_folder, exp_path, steps):
        ...
        with open(results_file, "w") as f:
            ...
            for nome, (X, y_real) in sets.items():
                ...
                # ----- MÉTRICAS SAZONAIS (apenas Teste) -----
                if nome == "Teste":
